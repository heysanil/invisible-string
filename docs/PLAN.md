...
10. **Proxy must forward `/eve/` AND `/.well-known/workflow/`** (callbacks hit `/v1/flow` and `/v1/step`) — else runs stall silently. `WORKFLOW_POSTGRES_JOB_PREFIX` isolates each agent's jobs in the shared world DB.
...
- **Compiled agent env (injected by supervisor, per agent):** `PORT`, `WORKFLOW_POSTGRES_URL`, `WORKFLOW_POSTGRES_JOB_PREFIX=<workflow_version_hash>`, `PLATFORM_JWT_SECRET` (channel auth), exactly one provider key (`OPENROUTER_API_KEY` | `ANTHROPIC_API_KEY`), `MCP_<CONN>_TOKEN…` (decrypted at launch), safety-cap values.
