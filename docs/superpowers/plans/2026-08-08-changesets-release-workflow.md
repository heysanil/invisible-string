...
> **EXECUTED — historical record; do not follow, do not edit.** This plan was carried out on the `chore/changesets-release-workflow` branch. Post-implementation review then **removed `changesets/action@v1`** (two defects, either one fatal here — see finding 8 of the design spec). Most of this plan shipped as written: the `images` job, the triggers, `fetch-depth: 0`, and the mise step are all current. What did **not** ship is what the yaml below shows for the `version` job's PR step (now a shell step), its `permissions`/`concurrency` blocks (now per-job, with an explicit `cancel-in-progress`), the `changesets/action@v1` pin, and every "Version Packages" PR title (the real title is `chore(release): version packages`). The current truth is `.github/workflows/release.yml` and revision 4 of the design spec; `AGENTS.md` → Releases is the operator-facing summary.
