...
/** `<mise install dir for the node pinned in mise.toml>/bin`, pinned on the
 *  worker's PATH. Config-driven (`where node`), never fuzzy (`where node@24`):
 *  the fuzzy form resolves to the newest 24.x mise knows about, which may not
 *  be the version mise.toml pins. */
