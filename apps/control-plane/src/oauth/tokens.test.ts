...
describe.skipIf(!TEST_DATABASE_URL)("oauth token lifecycle", () => {
  ...
  test("invalid_grant on refresh lands expired + auth_error and throws oauth_not_connected", async () => {
    ...
    const conn = (await connectionRow(id))!;
    // `auth_error`, not `auth_required`: a credential existed and the AS
    // rejected it. `auth_required` is for a grant nobody ever consented to.
    // The probe agrees — it reads the grant's status so the two cases do not
    // collapse into one badge (probe/service.ts).
