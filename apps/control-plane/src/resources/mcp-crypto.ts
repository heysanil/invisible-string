...
/**
 * Encrypt an auth WRITE for the rebuilt `connections` table, or `null` to
 * clear credentials. Same normalized plaintext shape as
 * {@link encryptMcpAuthConfig} (bearer `{type,token}` / headers
 * `{type,headers}` — one decrypt-side contract for the dispatch/compile
 * paths), but AAD-bound to the NEW table via {@link connectionAuthAad}.
 */
