...
await run(
  ["docker", "compose", "up", "-d", "--wait", "postgres", "garage", "dex", "meilisearch"],
  childEnv,
);
note(`infra healthy (postgres, garage, dex, meilisearch) · bucket ok  ${((Date.now() - infraStart) / 1000).toFixed(1)}s`);
