...
2. **`context: string[]` is not a `send()` option.** (0.19 form: `send(message, { auth, continuationToken, state?, title? })`; on 0.31 a custom channel handler gets `{from, resolveSession, attachSession, to, params, waitUntil, requestIp}` — no bare `send` — and channel-local continuation ADDRESSES are unrelated to the retired session-API tokens.) Context injection lives in the **eve channel's `onMessage` hook** (`return { auth, context: [...] }`). Custom trigger channels fold context blocks into the message content instead.
...
6. **eve requires Node 24.x** for agents; deps `eve`, `ai`, `zod`. eve's `ai` major is undocumented → resolved empirically in Phase 0; `@openrouter/ai-sdk-provider@2.x` needs `ai@^6` (usage: `createOpenRouter({apiKey})` then `openrouter('z-ai/glm-5.2')`), fallback lines exist for older majors. Seed slugs verified live: `z-ai/glm-5.2`, `deepseek/deepseek-v4-pro`, `deepseek/deepseek-v4-flash` ✓. ⚠️ **Seed slugs SUPERSEDED** (`docs/superpowers/specs/2026-08-08-reasoning-effort-and-model-defaults.md` §1): powerful → `moonshotai/kimi-k3` @ `max`, balanced/quick → `~deepseek/deepseek-v4-flash-latest` @ `max`/`low` — the same model at two efforts, which is why a preset now carries a **reasoning effort** as well as a model. The provider pin is also superseded (`@openrouter/ai-sdk-provider@3.0.0` on `ai@7`, spike REPORT finding 21), and on that provider the effort must ride the model's `extraBody`, not eve's `reasoning:` config (finding 29).
...
11. eve channel HTTP API. ⚠️ **SUPERSEDED by eve 0.31** (`docs/superpowers/specs/2026-08-07-eve-0.31-upgrade-design.md` §5; route table in `docs/runtime-worker-contract.md`): sessions are **ID-addressed**, continuation tokens are gone and are rejected with a 400 on every session route. `POST /eve/v1/session` → 202 `{ok, sessionId, status:"accepted"}`; `POST /eve/v1/session/:id` follow-ups take `message` XOR `inputResponses` and answer 409 `session_not_active` on an unknown/terminal id; control routes `/cancel`, `/clear`, `/compact`, `/reset`; `GET /eve/v1/session/:id/stream` NDJSON (stream version 21, `evt_`-ULID `meta.id`) with `?startIndex=` resume and optional `includeTailIndex=1`. Event types include `session.*, turn.*` (incl. `turn.cancelled`), `step.*, message.*, action.partial, context.cleared, input.requested, reasoning.*, authorization.*` — shapes frozen in `packages/shared/src/eve-events.ts` from live captures.
...
- **Session model:** chat thread = `agent_sessions` row = one eve session; store `{eve_session_id, workflow_version_id, affinity_worker_id, origin, principal}`. Follow-ups: `POST /eve/v1/session/:id` — addressed by the stored eve session id alone (0.31; the `continuation_token` column is dead-but-present, see AGENTS.md residuals). Runs = one per inbound message/trigger; `run_events` normalized from NDJSON.
...
- Follow-up message on the same session id shares session memory (0.19: via continuation token — superseded, see correction 11).
...
## Phase 7 — Workflow pipelines (2026-08-31)

**Design record:** `docs/superpowers/specs/2026-08-31-workflow-pipelines-design.md`. **Status: COMPLETE (2026-09-01), single delivery.** The workflow inverts from "one agent, one memo, one LLM turn" to a **pipeline the control plane interprets**: v2 config `{version: 2, trigger, steps[], onComplete?, overlap}` with `tool`/`infer`/`agent` steps plus `for_each`/`branch`/`filter`/`state` control verbs (`script` enum-reserved for a sandboxed Phase-5 slot). New interpreter in `apps/control-plane/src/pipeline/` over a durable `run_steps` ledger (`(run_id, path)` claims, advisory-locked driver, crash recovery by replay); `mode:'pipeline'` runs have no session (`runs.agent_session_id` NOT NULL dropped — the one relaxation — with the LEFT-join/COALESCE audit); agent steps spawn child runs via the extracted `dispatchRenderedRun`, sending `mode:"task"` + `outputSchema` (spike finding 36); delivery is explicit `onComplete.slackReply` only; `dispatchTriggerRun` and the `renderTaskMessage` workflow-dispatch path are removed with no v1 compatibility (the platform is undeployed; dev stacks reset). Copilot workflow surface pivots to granular step mutations + read tools; the SPA ships the two-pane composer/pipeline-strip editor and the Runs timeline. Supersedes the 2026-07-10 spec's workflow concept block, its "workflows attach no extra context", and its workflow copilot toolset.
