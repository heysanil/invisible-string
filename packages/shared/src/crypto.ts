...
export function encryptSecret(
  ...
  try {
    ...
    return {
      v: ENVELOPE_VERSION,
      edk: wrapDataKey(dataKey, masterKey),
...
export function decryptSecret(
  ...
  const dataKey = unwrapDataKey(edk, masterKey);
  try {
...
export function rotateEnvelope(
  ...
  const dataKey = unwrapDataKey(edk, oldMasterKey);
  try {
    return {
      v: ENVELOPE_VERSION,
      edk: wrapDataKey(dataKey, newMasterKey),
      iv: envelope.iv,
      tag: envelope.tag,
      ct: envelope.ct,
    };
...
function wrapDataKey(dataKey: Buffer, masterKey: MasterKey): string {
...
function unwrapDataKey(edk: Buffer, masterKey: MasterKey): Buffer {
