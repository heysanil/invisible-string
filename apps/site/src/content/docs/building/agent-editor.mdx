...
The Agent's name is inline-editable in the header: click in, type, and Enter commits (or click away to blur-commit); Escape reverts to the last saved name. Owners and admins also see a **Delete** action, which removes the Agent and every published version — blocked while any Workflow or chat session still references it.

**Chat with agent** always publishes first, even if the Agent is already published — for an unchanged draft that's a cache hit, not a rebuild. This is worth knowing precisely because it's easy to misread: clicking it chats against the Agent's latest **published** version, republishing the draft first if needed, never your unpublished draft directly. If the publish doesn't succeed cleanly, you get an error toast instead of a navigation. See [Chat](/docs/building/chat) for what happens once the session opens.
