...
Per AGENTS.md, cancelling a run whose eve session is already dead returns 200 `no_active_turn` — the same dead-session condition, not "nothing was running". If the control plane does **not** settle the run row to `canceled` in that case, `slotHeld` stays true forever: the flush never fires, give-up never triggers (it counts *flush* rejections, and no flush is attempted), and the queued text is stuck behind a phantom run with only the discard dialog as an escape. **Confirm the row settles on cancel during implementation.** If it cannot be guaranteed, the strip needs a manual "send now / discard" escape.
